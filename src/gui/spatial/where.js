...
createNameSpace("realityEditor.gui.spatial.where");

realityEditor.gui.spatial.where.objects = realityEditor.objects;
realityEditor.gui.spatial.where.locator = globalStates.whereIs.locator;
realityEditor.gui.spatial.where.screenLocation = {x:-1,y:-1};
realityEditor.gui.spatial.where.utilities = realityEditor.gui.ar.utilities;

realityEditor.gui.spatial.where.is = function(matrix, objectID,toolID,nodeID){
 if(this.locator.locateMe === true){
     if(this.locator.objectID === objectID){
         if(this.locator.toolID && this.locator.nodeID){
             // locate node 
             
         } else if (this.locator.toolID) {
             // locate tool
...
realityEditor.gui.spatial.where.draw = function (matrix){
   ...
   // ctx.clearRect(0, 0, globalStates.width, globalStates.height);
