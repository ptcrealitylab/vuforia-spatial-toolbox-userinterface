...
// This gives correct final result but doesn't let you write to it because it returns a copy
/*
realityEditor.gui.ar.positioning.getPositionData = function(activeVehicle) {
    var positionDataSource = activeVehicle;
    if (activeVehicle.hasOwnProperty('visualization')) {
        positionDataSource = activeVehicle.ar; //(activeVehicle.visualization === "ar") ? (activeVehicle.ar) : (activeVehicle.screen);
        return positionDataSource;
    }
    
    var positionData = {};
    positionData.x = positionDataSource.x;
    positionData.y = positionDataSource.y;
    positionData.scale = positionDataSource.scale;
    positionData.matrix = positionDataSource.matrix;
    ...
    if (activeVehicle.type === 'node' || activeVehicle.type === 'logic') {
        ...
        if (frame) {
            ...
            positionData.x += parentFramePositionData.x;
            positionData.y += parentFramePositionData.y;

            if (parentFramePositionData.matrix.length === 16) {
                
                if (positionData.matrix.length === 16) {
                    var result = [];
                    realityEditor.gui.ar.draw.utilities.multiplyMatrix(parentFramePositionData.matrix, positionData.matrix, result);
                    positionData.matrix = result;
                
                } else {
                    positionData.matrix = parentFramePositionData.matrix;
                }
                //
...
realityEditor.gui.ar.positioning.getPositionData = function(activeVehicle) {
    ...
    if (activeVehicle.type === 'node' || activeVehicle.type === 'logic') {
        ...
        if (frame) {
            ...
            if (parentFramePositionData.matrix.length === 16) {
                if (activeVehicle.matrix.length === 16) {
                    // both have matrix -> multiply
                    activeVehicle.combinedPosition.matrix = [];
                    realityEditor.gui.ar.draw.utilities.multiplyMatrix(parentFramePositionData.matrix, activeVehicle.matrix, activeVehicle.combinedPosition.matrix); 
                } else {
                    // only parent frame has matrix -> just use that
                    activeVehicle.combinedPosition.matrix = parentFramePositionData.matrix;
                }
            } else {
                // only this node has matrix -> just use that
                activeVehicle.combinedPosition.matrix = activeVehicle.matrix;
            }
        }
    }
